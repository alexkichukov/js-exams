...
const patternOne_oneliner = (length: number) => console.log(Array(length).fill(1).map((_, index) => Array(index + 1).fill(1).map((_, index) => index + 1).join(' ')).join('\n'))
const patternTwo_oneliner = (length: number) => console.log(Array(length *  2 - 1).fill(1).map((_, index) => Array(index >= length ? length * 2 - 1 - index : index + 1).fill(1).map((_, index) => index + 1).join(' ')).join('\n'))
...
function patternTwo(length: number) {
  for (let i = 1; i <= length * 2 - 1; i++) {
    let line = '';
    for (let j = 1; j <= (i > length ? length * 2 - i : i); j++)
      line = line + j + ' ';
    console.log(line);
  }
}
...
function patternFour(length: number) {
  const lines: string[] = [Array(length).fill(1).map((_, i) => i + 1).join(' ')];
  for (let i = 1; i < length; i++) {
    const line = ' '.repeat(i) + Array(length - i).fill(1).map((_, i) => i + 1).join(' ') + ' '.repeat(i);
    lines.unshift(line);
    lines.push(line);
  }
...
function patternSix(length: number) {
  ...
  // Print first line
  console.log(Array(length).fill(1).map((_, i) => pad(`${i + 1}`)).join(' '));
  
  // Print the lines between the first and last
  let left = length * 4 - 4;
  let right = length + 1;
  for (let i = 0; i < (length - 2); i++) {
    console.log(pad(`${left--}`) + ' '.repeat(maxWidth * (length - 2) + length - 1) + pad(`${right++}`));
  }

  // Print the last line
  console.log(Array(length).fill(left).map((n, i) => pad(`${n - i}`)).join(' '));
}
